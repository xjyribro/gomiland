...
class Brocky extends Npc with HasGameReference<GomilandGame> {
  Brocky({required super.position});
  ...
  Future<void> startConversation(Vector2 playerPosition) async {
    ...
    yarnProject
        .parse(await rootBundle.loadString(Assets.assets_yarn_brocky_yarn));
    DialogueRunner dialogueRunner = DialogueRunner(
        yarnProject: yarnProject, dialogueViews: [dialogueControllerComponent]);
    await dialogueRunner.startDialogue(getZenGardenSellerDialogue(zenGarden, 'brocky'));
    game.unfreezePlayer();
  }
}
