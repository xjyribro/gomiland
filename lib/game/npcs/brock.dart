...
class Brock extends Npc with HasGameReference<GomilandGame> {
  Brock({required super.position});
  ...
  Future<void> startConversation(Vector2 playerPosition) async {
    ...
    yarnProject
        .parse(await rootBundle.loadString(Assets.assets_yarn_brock_yarn));
    DialogueRunner dialogueRunner = DialogueRunner(
        yarnProject: yarnProject, dialogueViews: [dialogueControllerComponent]);
    await dialogueRunner.startDialogue(getZenGardenSellerDialogue(zenGarden, 'brock'));
    game.unfreezePlayer();
  }
}
